...
import getElementStyles from './utils/get-element-customized-styles.js';
import { modules as configModules } from './custom-build-config.js';
...
export default async function buildModules() {
  ...
  const coreElementContent = fs
    .readFileSync('./src/swiper-element-customized.mjs', 'utf-8')
    .replace(`import Swiper from './swiper.mjs';`, `import Swiper from './swiper-bundle.mjs';`);
  fs.writeFileSync('./src/swiper-element-customized-bundle.mjs', coreElementContent);
  ...
  const output = await rollup({
    ...
    input: [
      ...
      './src/swiper-element-customized.mjs',
      './src/swiper-element-customized-bundle.mjs',
      './src/swiper-vue.mjs',
  ...
  await Promise.all(
    [
      'swiper-bundle.mjs',
      'swiper.mjs',
      'swiper-element-customized.mjs',
      'swiper-element-customized-bundle.mjs',
    ].map(async (f) => {
      const output = await rollup({
        input: `./dist/${f}`,
        plugins: [
  ...
  if (isProd) {
    fs.unlinkSync('./src/swiper-element-customized-bundle.mjs');
  }
  ...
  await Promise.all([
    ...
    ...[
      'swiper-bundle.js',
      'swiper.js',
      'swiper-element-customized.js',
      'swiper-element-customized-bundle.js',
    ].map((f) => {
