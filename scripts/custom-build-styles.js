  ...
  if (isProd) {
    ...
    await Promise.all(
      modulesLessFiles.map(async (filePath) => {
        ...
        await fs.writeFile(`${resultFilePath}-element-customized.css`, resultCSSElement);
        await fs.writeFile(`${resultFilePath}.min.css`, minifiedCSS);
        await fs.writeFile(`${resultFilePath}-element-customized.min.css`, minifiedCSSElement);
      }),
    );
  }
